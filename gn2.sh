...
# Find File.
# KEYWORDS="*translated|*translating|*fanyi|*??????|*???뷭??"
# cd $PWD/../TranslateProject/sources/tech/
# grep -RHEvi ${KEYWORDS} * | grep -Ev "LCTT?????淶.md|.git|README" | \
# grep -RHEvi ${KEYWORDS} * | grep -Ev "$(find . -type d | sed 's#[^/]*/##' | \
# awk -F '[/]' '{print $2}')|LCTT?????淶.md|.git|README" | \
...
# Show File.
  ...
  for i in translated translating fanyi ?????? ???뷭?? 
  do
